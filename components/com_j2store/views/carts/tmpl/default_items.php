					...
					<td>
						...
						<span class="cart-product-name">
							<?php echo $item->orderitem_name; ?>
							 <?php if(!$this->params->get('show_qty_field', 1)) : ?>
							 <a class="j2store-remove remove-icon" href="<?php echo $platform->getCartUrl(['task' => 'remove', 'cartitem_id' => $item->cartitem_id]); ?>">X</a>
							 <?php endif; ?>
						</span>
						<br>
						<?php if(isset($item->orderitemattributes) && $item->orderitemattributes): ?>
							...
							<?php foreach ($item->orderitemattributes as $attribute): ?>
								<small>
								- <?php echo Text::_($attribute->orderitemattribute_name); ?> : <?php echo nl2br((string) Text::_($attribute->orderitemattribute_value)); ?>
								</small>
								<br>
							<?php endforeach;?>
							</span>
					  ...
					  <?php if($this->params->get('show_qty_field', 1)) : ?>
						<td>
							<?php echo J2Store::product()->displayQuantity('com_j2store.carts', $item, $this->params, ['class'=>'input-mini '] ); ?>
							<a class="btn btn-small btn-danger btn-xs j2store-remove remove-icon" href="<?php echo $platform->getCartUrl(['task' => 'remove', 'cartitem_id' => $item->cartitem_id]); ?>">
