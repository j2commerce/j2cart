...
<h3><?php echo Text::_('J2STORE_ORDER_SUMMARY')?></h3>
	<div class="alert alert-danger"><?php echo Text::_ ( 'J2STORE_ORDER_EDIT_SUMMARY_TAX_WARRING_MESSAGE' );?></div>
	<?php echo J2Store::plugin ()->eventWithHtml ( 'BeforeAdminOrderSummery', [&$order, &$items] );?>
				...
				<tr>
					<td>
						...
						<?php if(isset($item->orderitemattributes)): ?>
							...
							?>
								<small>
								- <?php echo Text::_($attribute->orderitemattribute_name); ?> : <?php echo $attribute_value; ?>
								</small>

								<!--link to download for files-->
             				   <?php if(J2Store::platform()->isClient('administrator') && $attribute->orderitemattribute_type=='file' && Factory::getApplication()->input->getString('task')!='printOrder'):?>

             					  <a target="_blank" class="btn btn-primary"
             					  href="<?php echo Route::_('index.php?option=com_j2store&view=orders&task=download&ftoken='.$attribute->orderitemattribute_value);?>"
             					  >
...
<?php endforeach; ?>
		...
		<?php $colmspan = 3;?>
			...
			<?php $colmspan = 2;?>
			<tr>
				<td colspan="<?php echo $colmspan;?>"><?php echo Text::_('J2STORE_CART_SUBTOTAL'); ?>
				</td>
			...
			<tr>
				<td colspan="<?php echo $colmspan;?>"><?php echo Text::_(stripslashes((string) $this->shipping->ordershipping_name)); ?>
				</td>
			...
			<tr>
				<td colspan="<?php echo $colmspan;?>"><?php echo Text::_('J2STORE_ORDER_SHIPPING_TAX'); ?>
				</td>
			...
			<tr>
				<td colspan="<?php echo $colmspan;?>"><?php echo Text::_($fee->name); ?><a class="j2store-remove remove-icon" href="javascript:void(0)" onClick="removeFee('<?php echo $fee->j2store_orderfee_id;?>')">X</a>
				</td>
			...
			<tr>
				<td colspan="<?php echo $colmspan;?>"><?php echo Text::_('J2STORE_CART_SURCHARGE'); ?>
				</td>
			...
			<?php foreach($this->order->getOrderDiscounts() as $discount):?>
					...
					<tr>
						...
						<?php if($discount->discount_type == 'coupon'):?>
							<?php echo Text::sprintf('J2STORE_COUPON_TITLE', $discount->discount_title); ?>
						...
						<?php elseif($discount->discount_type == 'voucher'):?>
							<?php echo Text::sprintf('J2STORE_VOUCHER_TITLE', $discount->discount_title); ?>
							<a class="j2store-remove remove-icon" href="javascript:void(0)" onClick="removeVouchers()">X</a>
						<?php else:?>
							<?php echo Text::sprintf('J2STORE_DISCOUNT_TITLE', $discount->discount_title); ?>
						<?php endif;?>
						</td>
			...
			<?php endforeach;?>
			<!-- taxes -->
			<?php if(isset($this->taxes) && (is_countable($this->taxes) ? count($this->taxes) : 0) ): ?>

				<?php foreach ($this->taxes as $tax):?>
					<tr>
						<td colspan="<?php echo $colmspan;?>"><?php if($this->params->get('checkout_price_display_options', 1)):?>
									<?php echo Text::sprintf('J2STORE_CART_TAX_INCLUDED_TITLE', $tax->ordertax_title, floatval($tax->ordertax_percent).'%');?>
							<?php else:?>
									<?php echo Text::sprintf('J2STORE_CART_TAX_EXCLUDED_TITLE', $tax->ordertax_title, floatval($tax->ordertax_percent).'%');?>
							<?php endif;?>
						</td>
						<td>
			...
			<tr>
				<td colspan="<?php echo $colmspan;?>"><?php echo Text::_('J2STORE_CART_REFUND');?></td>
				<td><?php echo $this->currency->format($this->order->order_refund,$this->order->currency_code, $this->order->currency_value);?></td>
			</tr>
			<?php endif;?>
			<tr>
				<td colspan="<?php echo $colmspan;?>"><?php echo Text::_('J2STORE_CART_GRANDTOTAL'); ?>
				</td>
			...
			<tr class="add_fee_con">
				...
				<td colspan="<?php echo $colmspan+1;?>">
					...
					echo J2Html::select()->clearState()
						...
						->value('')
						->setPlaceHolders([''=>Text::_('J2STORE_NOT_TAXABLE')])
						->hasOne('Taxprofiles')
						->setRelations(
							['fields' => ['key'=>'j2store_taxprofile_id', 'name'=>'taxprofile_name']]
						)->getHtml();
					?>
					<input type="button" id="add_additional_fee" onclick="addAdditionalFee()" class="btn btn-warning" value="<?php echo Text::_('J2STORE_ADD_ADDITIONAL_FEE');?>"/>
				</td>
			</tr>
			<tr>
				<td colspan="<?php echo $colmspan+1;?>" >
					<span class="pull-right">
						<button id="calculate_tax" class="btn btn-warning"><?php echo Text::_('J2STORE_CALCULATE_TAX');?></button>
					</span>
				</td>
			</tr>
		</tfoot>
